      ...
      <main className="pt-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
          <nav className="flex items-center gap-2 text-xs text-brand-gray">
            ...
            <span>/</span>
            <span className="text-brand-mid capitalize">{product.category}</span>
            <span>/</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
              ...
              <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-white/5">
                <div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${product.colorHex}22 0%, ${product.colorHex}44 40%, ${product.colorHex}11 100%)`,
                  }}
                />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div
                      className="w-24 h-24 rounded-full mx-auto mb-4 opacity-40"
                      style={{ backgroundColor: product.colorHex }}
                    />
                    <p className="text-xs text-brand-gray font-mono">{product.brand}</p>
                  </div>
                </div>
              ...
              <div className="flex flex-col justify-center">
                <div className="mb-6">
                  <span className="text-xs tracking-widest uppercase text-brand-mid font-mono">
                    {product.brand}
                    {product.category && ` / ${product.category}`}
                    {product.era && ` / ${product.era.replace('-', ' ')}`}
                  </span>
                  <h1 className="font-display text-3xl sm:text-4xl text-brand-cream mt-2 mb-2">{product.name}</h1>
                  {product.episode && (
                    <p className="text-xs text-brand-yellow font-mono mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {product.tags.map((tag) => (
                    <span key={tag} className="text-xs text-brand-gray bg-white/5 border border-white/5 px-3 py-1 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <a
                  ...
                  className="w-full block text-center bg-brand-yellow text-brand-black py-4 rounded-lg text-sm font-bold tracking-wide hover:bg-brand-amber transition-colors mb-3"
                >
                  Buy on {sourceName} — ${product.price}
                </a>
                <Link
                  href="/shop"
                  className="w-full block text-center border border-white/20 text-brand-cream py-4 rounded-lg text-sm tracking-wide hover:bg-white/5 transition-colors"
                >
                  Back to Shop
                </Link>
                ...
                {product.affiliateNote && (
                  <p className="text-[11px] text-brand-amber mt-4 leading-relaxed">
                    {product.affiliateNote}
                  </p>
                )}
                <p className="text-[10px] text-brand-gray mt-3 leading-relaxed">
                  As an Amazon Associate and affiliate partner, Jesse&apos;s Closet earns from qualifying purchases.
                  Links may earn us a commission at no extra cost to you. All recommendations are editorial and independently curated.
                </p>
              </div>
            </div>
          </div>
